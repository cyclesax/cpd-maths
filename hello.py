...
print(
    mytuple.index(2000)
)  # should return 2 as 2000 is the last item in 0-based positioning
...
# Part 10: 4 December: main() function and parameters from command line
# ======================================================================
...
# Part 12: 6 December SQLite
# ==========================

# SQlite is the standard file-based database and is supported by Python
import sqlite3

# Create a new database if it does not exist or open it if it does
connection = sqlite3.connect("test.db")

# A cursor is the object we use to manipulate the database
cursor = connection.cursor()

# Remove existing tables so we start from scratch
cursor.execute("DROP TABLE IF EXISTS movies")

# Create an empty table with default field types
cursor.execute("CREATE TABLE movies(title, year, score)")

# Stick some data into the table
cursor.execute(
    """
    INSERT INTO movies VALUES
        ('Apollo 13', 1995, 'the best film'),
        ('Brassed off', 1996, 11),
        ('The Lake House', 2006, 10)
"""
)

# This is an important function to execute to save the data into
# the database.  Not required for some other SQLite uses but does
# appear to be necessary here or data is not stored to disk even
# though it can be queried
connection.commit()

# A query that selects some of the records and this gives us another
# cursor object that we are calling results here and this contais the
# active query that we can then interact with
results = cursor.execute("SELECT * FROM movies WHERE year > 1995")
print("results",results, type(results))

# Now to get results from the cursor we grab some or all to the variable
# data which is a list of tuples
data = results.fetchall()
print("data",data, type(data), type(data[0]))

# So now to get at the individual items of data we unwrap the tuple
filmname = data[0][0]
filmyear = data[0][1]
filmscore = data[0][2]

print(filmname, type(filmname)) # a string
print(filmyear, type(filmyear)) # an int
print(filmscore, type(filmscore)) # an int

# We know from the definition of the score for Apollo 13 that we could
# get a string in that last field
results = cursor.execute("SELECT * FROM movies WHERE year = 1995")
data = results.fetchall()
filmscore = data[0][2]
print(filmscore, type(filmscore)) # a string

# If we try this to see the type of field for each record then
# we get back text, integer, integer : which means that each
# of the records is some type of variant.  It would therefore
# likely be better/more efficient/faster to declare the type
# with something like this and to stick to those types as per
# classic database design
cursor.execute("DROP TABLE IF EXISTS movies2")
cursor.execute("CREATE TABLE movies2(title TEXT, year INTEGER, score INTEGER)")
cursor.execute(
    """
...
# BUT this appears not to work as expected and the string 'the best film' ends
# up in the table in a column that's supposed to be for integers.
